...
- **Implementation planning (default)** — planning code/features. Requirements MUST exist in `.co-agents/requirements/`; if they don't, stop and tell the user to run `/co-spec` first. Follow the full workflow below.
- **Lightweight planning** — exploratory, research, or non-code work (a research roadmap, an experiment plan, a writing/delivery plan), or when the user explicitly asks for a quick plan. Skip the requirements precondition and the code-specific gates: produce a phased task list using effort/uncertainty for complexity, drop `Approach: TDD` and REQ-ID coverage where they don't apply, and note that this plan isn't backed by formal requirements. Use this when the work doesn't warrant the full spec→plan ceremony.

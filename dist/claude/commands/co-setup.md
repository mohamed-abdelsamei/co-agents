---
description: "Set up a new project — define constitution, configure CLAUDE.md, and initialize project memory."
...
The user wants to set up a new project: **$ARGUMENTS**
...
- `CLAUDE.md` — **only** the `Language & Stack Focus` and `Code Quality` sections (preserve everything else)
...
1. **Parse input** — Extract project name, purpose, stack, and constraints from `$ARGUMENTS`.
...
3. **Scan for existing files** — If `.co-agents/constitution.md` or `CLAUDE.md` already have content, present them for review instead of overwriting blindly.
...
6. **Update `CLAUDE.md`** — Set the `Language & Stack Focus` section:
...
- Do NOT modify routing, memory, or agent sections of `CLAUDE.md`.
- If the user provides enough info in `$ARGUMENTS`, skip redundant questions and go straight to producing documents.
- If a constitution already exists with `status: active`, warn before overwriting.

## Done When

`constitution.md` is written, `CLAUDE.md` stack section is configured, and a summary is provided. Then suggest `/co-spec` to define the first feature.

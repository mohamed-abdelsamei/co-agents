...
> Act as the **critic** agent (`${CLAUDE_PLUGIN_ROOT}/agents/critic.md`).

The user wants to be grilled on: **$ARGUMENTS**
...
Identify the target — a decision (`decisions.md`), plan (`tasks/`), requirements (`requirements/`), research finding (`research/`), or a raw idea in `$ARGUMENTS` — and load any relevant context. If it's a bare idea, grill from the user's stated reasoning. Open with the single most important question; one clarifying question is fine if the target is unclear.

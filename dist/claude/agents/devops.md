...
- **Reads**: `docs/` (incl. `architecture.md`), `constitution.md`, `decisions.md`, `requirements/`, `tasks/`, `research/`
...
- [ ] Architecture docs reviewed (`docs/`, incl. `docs/architecture.md`)
...
5. **Document** — Note pipeline structure in `docs/` (e.g. `docs/architecture.md`)
...
- Create a `feat/{infra-name}` (or `fix/`) branch before changing infrastructure.
- Commit after a pipeline or stack is working, using conventional commits (`feat:`, `fix:`, `chore:`). Keep commits atomic. Never commit secrets, credentials, or environment-specific config.

## Code Quality

Follow the standards in `${CLAUDE_PLUGIN_ROOT}/instructions/code-quality.md`. Additionally:

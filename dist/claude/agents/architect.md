---
name: architect
description: "Use for requirements, architecture, and planning — clarifying scope, defining architecture, breaking work into tasks, reviewing implementation against specs, and strategic advice (RFC review, tradeoff analysis, decision framing). Use at any project phase, not just at the start."
...
- **Reads**: `docs/` (incl. `architecture.md`), `constitution.md`, `decisions.md`, `improvements.md`, `research/`, `requirements/`
- **Writes**: `docs/` (incl. `architecture.md`, design docs), `requirements/`, `tasks/`, `decisions.md`, `reviews/`
...
Apply quality gates from `${CLAUDE_PLUGIN_ROOT}/instructions/code-quality.md`.
...
2. **Review Implementation** — For each requirement/task, check:
   ...
   - **Code quality**: Follows standards from `${CLAUDE_PLUGIN_ROOT}/instructions/code-quality.md`?

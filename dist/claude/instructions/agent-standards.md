...
- Don't re-read large or static files you've already loaded this session — but DO re-read mutable artifacts (`tasks/`, `requirements/`, files you or another agent just edited) when their contents may have changed
...
- **Cascade changes**: When a requirement is rewritten or dropped, update all referencing tasks per the cascade rules in `memory.md`.
- **One file per feature**: Never create a second file for the same feature or topic.
- Full memory conventions are in `${CLAUDE_PLUGIN_ROOT}/instructions/memory.md`.
